...
export default function ManagedeliveryTable() {
  const GET_URL = `${deliveryBoyRequests.getAllDelivery}`;
  ...
  const GetData = async () => {
    try {
      await Service.GetDelivery().then((res) => {
        setLoading(true);
  ...
  const deleteData = (dataid) => {
    Service.DeleteDeliveryBoy(dataid)
      .then((res) => {
        child();
        handleClose();
      })
