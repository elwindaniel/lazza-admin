...
import Form from "../../../common/forms/createCity";
...
export default function DistrictTable() {
  ...
  const GetData = async () => {
    try {
      await Service.GetCity().then((res) => {
        setLoading(true);
    ...
    setFilterfn({
      fn: (items) => {
        if (target.value == "") return items;
          ...
          return items.filter(
            (x) =>
              x.cityName.toLowerCase().includes(target.value) ||
              x.cityName.toUpperCase().includes(target.value) ||
              x.cityName.includes(target.value) ||
  ...
  const editData = (editId, editName, editDistrict, editState, editCountry) => {
    setOpen(true);
    setEdit(editId);
    setEditName(editName);
    setEditDistrictId(editDistrict);
